...
export default function ResourcesPage() {
  ...
  const [form, setForm] = useState({ title: '', type: 'notes', subject: '', description: '', linkUrl: '' });
  ...
  const createResource = async (e) => {
    ...
    setForm({ title: '', type: 'notes', subject: subjects[0]?._id || '', description: '', linkUrl: '' });
    load();
  };
    ...
    <div>
      ...
      {canUpload && (
        <form onSubmit={createResource} className="card mb-4 grid gap-2 md:grid-cols-2">
          ...
          <input className="rounded-xl border p-2" placeholder="File/Reference URL" value={form.linkUrl} onChange={(e) => setForm({ ...form, linkUrl: e.target.value })} />
