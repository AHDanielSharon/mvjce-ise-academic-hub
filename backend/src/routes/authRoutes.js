...
import { getApprovedStaffAccount, isApprovedStaffEmail } from '../config/staffRegistry.js';
...
const getStaffPortalError = (email, portal) => {
  if (!email) return 'Email is required.';
  if (!isApprovedStaffEmail(email)) {
    return `Only approved official MVJCE staff accounts can use the ${portal} portal. Contact management if your staff account is missing.`;
  }
...
router.post('/register', async (req, res) => {
  try {
    ...
    const approvedStaff = getApprovedStaffAccount(normalizedEmail, role);
    if (!approvedStaff) {
      return res.status(403).json({ message: 'Staff self-registration is restricted to approved official MVJCE accounts only.' });
    }
    ...
    const payload = {
      ...
      section: approvedStaff.section,
      designation: approvedStaff.designation
    };

    if (!isDatabaseReady()) {
      return res.status(400).json({ message: 'This official staff account already exists. Please login with email and password.' });
    }

    const exists = await User.findOne({ email: normalizedEmail });
    if (exists) return res.status(400).json({ message: 'This official staff account already exists. Please login with email and password.' });
...
router.post('/login', async (req, res) => {
  try {
    ...
    if (!isDatabaseReady()) {
      ...
      if (!offlineUser) return res.status(400).json({ message: getStaffPortalError(normalizedEmail, portal) });
    ...
    if (!isApprovedStaffEmail(normalizedEmail)) {
      return res.status(403).json({ message: getStaffPortalError(normalizedEmail, portal) });
    }
